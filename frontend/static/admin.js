    ...
    function renderSessions(sessions) {
        ...
        sessions.forEach(session => {
            ...
            card.innerHTML = `
                ...
                <div class="session-actions" style="margin-top: 1.5rem; display: flex; gap: 0.75rem; flex-wrap: wrap;">
                    ...
                    <a href="${API_BASE_URL}/api/sessions/${session.session_id}/zip" class="btn" style="flex: 0 0 auto; background: var(--success); color: white; border-radius: 12px; height: 42px; width: 42px; padding: 0; justify-content: center;" title="Download Full Session ZIP">
            ...
            deleteBtn.addEventListener('click', async () => {
                if (confirm('Are you sure you want to delete this session and all its photos?')) {
                    try {
                        const res = await fetch(`${API_BASE_URL}/api/sessions/${session.session_id}`, { method: 'DELETE' });
                        if (res.ok) {
        ...
        document.querySelectorAll('.photo-item').forEach(item => {
            item.addEventListener('click', () => {
                ...
                const downloadUrl = img.src.replace('/admin/photo/', `${API_BASE_URL}/api/photo/download/`);
